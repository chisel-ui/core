...
Colors('CSSColorNames', () => {
    assert.ok(Array.isArray(colors.validCSSColorNames), 'validCSSColors should be an array')
    assert.type(colors.isValidColorName, 'function', 'isValidColorName should be a function')
    assert.ok(colors.isValidColorName('red'), '"red" is a valid color name')
    assert.not.ok(colors.isValidColorName('testing'), '"testing" is an invalid color name')
})
